...
const removeFromFav=async(req,res)=>{
    ...
    try {
        ...
        const favoriteStadiumIds = user.favorites;

        const favoriteStadiums = await Stadium.find({ _id: { $in: favoriteStadiumIds } });

        await user.save();
        res.json({
            success:"true",
            message:"item removed From favourite",
            favorites:favoriteStadiums
        })
    } catch(error) {
        res.json({
            success:"false",
...
const getUserFav=async (req,res,next)=>{
    ...
    // Fetch stadium details using the IDs
    const favoriteStadiums = await Stadium.find({ _id: { $in: favoriteStadiumIds } });

    return res.json({
        status:"Done",
        data:favoriteStadiums
    });
}
module.exports={
    getStadiums,
